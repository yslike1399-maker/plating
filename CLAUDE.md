...
**Single-component, many-useState pattern.** `App` (function body lines 159–14938) declares ~94 `useState` hooks at the top, one cluster per domain (products, history, vendors, salary, etc.). Each cluster has a paired `saveX(data)` helper that does both `setX(data)` and `localStorage.setItem('<key>', JSON.stringify(data))` — currently 31 such helpers exist (the `save*` block runs roughly L1372–L1448, just after `saveHistory`). When adding a new domain, follow the same triple: `useState` initializer that lazily reads localStorage, plus a `save*` helper. Do **not** call `setState` and `localStorage.setItem` separately at call sites — always go through the `save*` helper so the change-tracking counter (see below) stays in sync. (As of laceroom앱 정리 Phase 4-E, all previously-known violations — `vendors`, `vendor_bills`, `vendor_payments`, `vendor_items`, `fabric_orders`, `dashboard_links` — have been migrated to their `save*` helpers.)
